        ...
        <aside className="w-[210px] shrink-0 bg-sidebar border-r border-line flex flex-col">
          <div className="px-4 py-4">
            <div className="text-sm font-bold text-ink leading-tight">
              {gebruiker?.naam || t("app.gebruiker")}
            </div>
            <div className="text-xs text-muted mt-0.5">
              {gebruiker?.bedrijf || t("app.bedrijf")}
            </div>
          </div>
          ...
          {/* Uitloggen onderaan de zijbalk */}
          <div className="px-2 pt-2 border-t border-line">
            <button
              type="button"
              onClick={handleLogout}
              className="w-full flex items-center gap-3 rounded-md px-3 py-2 text-sm text-muted hover:bg-hover hover:text-ink transition-colors"
            >
              <LogoutIcon />
              <span className="flex-1 text-left">{t("auth.uitloggen")}</span>
            </button>
          </div>
